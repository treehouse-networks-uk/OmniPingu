...
class OmniPingSetUp():
    ...
    content = [
        ...
        '''Note : OmniPingu uses the “unsync” library to manage the asynchronous tests
        as opposed to the standard library async, await and asyncio loop. Aside from this
        it is identical to OmniPing'''
    ]
...
class OmniPingTestEng():
    ...
    @cherrypy.tools.json_out()
    def POST(self):
        action = cherrypy.request.json.get('action', '').lower()
